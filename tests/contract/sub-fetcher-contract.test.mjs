...
test('launcher has download-first actions and does not ask for an arbitrary URL', () => {
  ...
  assert.match(launcher, /function openPanel\(trigger\)[\s\S]*?syncCacheSession\(getAccessToken\(\)\)/);
  assert.match(launcher, /function closePanel\(\)[\s\S]*?\.sf-code[\s\S]*?textContent\s*=\s*['"]['"]/);
  assert.match(launcher, /actionEpoch|panelEpoch|operationId/);
  assert.match(launcher, /response = await window\.fetch[\s\S]*?cacheToken !== token[\s\S]*?response\.status/);
